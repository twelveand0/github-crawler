...
class DB_RepositoryEvent:
    """
        This class represents DB_RepositoryEvent as a class describle one github.Event.Event of one \
    ...
    repo = None
    db = None
    table = "RepositoryEvent"


    def __init__(self, event, repo, db):
        self.event = event
        self.repo = repo
        self.db = db
    ...
    def save(self):
        try:
            ...
            D = [(a,v) for (a, v) in self.event.__dict__.iteritems() if \
            ...
            for (attr, value) in D:
                ...
                if isinstance(v, bool):
                    ...
                    sql_fields = sql_fields + attr[1:] + ","
                    sql_values = sql_values + "'" + str(v).replace("'", "\\'").replace('"', '\\"') + "',"
    ...
    def exist(self):
        try:
            ...
            sql = "select count(*) from %s where id='%s';"%(self.table, self.event.id)
