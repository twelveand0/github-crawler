...
class DB_PullRequestPart:
    """
        This class represents DB_PullRequestPart as a database class for github.PullRequestPart.PullRequestPart
    """

    pullpart = None
    db = None
    table = "PullRequestPart"

    def __init__(self, pullpart, db):
        self.pullpart = pullpart
        self.db = db
    ...
    def save(self):
        try:
            ...
            D = [(a,v) for (a, v) in self.pullpart.__dict__.iteritems() if \
    ...
    def exist(self):
        try:
            ...
            sql = "select count(*) from %s where sha='%s';"%(self.table, self.pullpart.sha)
