...
class DB_PullRequestComment:
    """
        This class represents DB_PullRequestComment as a database class for github.PullRequestComment.PullRequestComment
    """
    ...
    def __init__(self, comment, pull, db):
    ...
    def save(self):
        try:
            ...
            sql_fields = sql_fields + "pull,"
            sql_values = sql_values + str(self.pull.id) + ","
    ...
    def update_one(self, field, value):
        try:
            ...
            if isinstance(value, basestring):
                sql = "update %s set %s='%s' where id=%d;"%(self.table, field, value, self.comment.id)
            elif isinstance(value, int):
                sql = "update %s set %s=%d where id=%d;"%(self.table, field, value, self.comment.id)
