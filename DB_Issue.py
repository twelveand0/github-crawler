...
class DB_Issue:
    """
        This class represents DB_Issue as a database operating class for github.Issue.Issue
    """
    ...
    def __init__(self, issue, repo, db):
        self.issue = issue
        self.repo = repo
        self.db = db
    ...
    def save(self):
        try:
            ...
            D = [(a,v) for (a, v) in self.issue.__dict__.iteritems() if \
            ...
            for (attr, value) in D:
                ...
                elif isinstance(v, github.IssuePullRequest.IssuePullRequest):
                    sql_fields = sql_fields + attr[1:] + ","
                    sql_values = sql_values + "'" + v.html_url + "',"
    ...
    def exist(self):
        try:
            ...
            sql = "select count(*) from %s where id=%d;"%(self.table, self.issue.id)
