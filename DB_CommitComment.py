...
class DB_CommitComment:
    """
        This class represents DB_CommitComment as a database operating class for github.CommitComment.CommitComment
    """
    ...
    def __init__(self, commit, comment, db):
        self.commit = commit
    ...
    def exist(self):
        try:
            ...
            sql = "select count(*) from %s where id=%d && commit_id='%s';"%(self.table, self.comment.id, self.commit.sha)
